...
import SubmitAppealDialog from "./submit-appeal-dialog";
...
export default function UserFlaggedPost({ flag }: Props) {
  const reason = useMemo(() => {
    return flagCategories.filter((cat) => cat.value === flag.report?.reason)[0]
      .label;
  }, [flag]);
  return (
    <div className="p-4 shadow-[-3px_0px] shadow-amber-800 bg-white flex flex-col items-start gap-3 rounded-lg border border-line w-full">
      <div className="flex items-center gap-4 justify-between w-full">
        <div className="flex items-start gap-2">
          <div className="p-2 text-red-800 bg-red-100 rounded-md">
            <FlagIcon className="size-4" />
          </div>
          <div className="flex flex-col items-start gap-0">
            <p className="font-semibold text-sm">
              Your post was flagged and removed
            </p>
        ...
        <Badge
          size={"xs"}
          variant={"destructive"}
          className="rounded-sm h-5 pointer-events-none text-red-800"
        >
          removed
        </Badge>
      </div>
      <div className="p-4 rounded-md bg-red-100 border border-red-200 flex flex-col items-start gap-2 w-full">
        <div className="flex items-center gap-2">
          <FlagIcon className="size-3 text-red-800" />
      ...
      <div className="flex items-center gap-4 justify-between w-full">
        <div className="flex items-center gap-2">
          {flag.report?.id && <SubmitAppealDialog reportId={flag.report?.id} />}
        </div>
        <span className="text-xs opacity-50">
